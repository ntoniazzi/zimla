...
class RC4 {
     ...
     */
    public getStringBytes(string: string): Array<number> {
        let output: Array<number> = [];
        for (let i = 0, m = string.length; i < m; i++) {
    ...
    private mix(seed: string): void {
        let input = this.getStringBytes(seed);
        let l = input.length;
        let j = 0;
        for (let i = 0, m = this.s.length; i < m; i++) {
            j += this.s[i] + input[i % l];
            j %= 256;
            this._swap(i, j);
        }
    }
...
class RNG {
     ...
     * @returns {Function}
     */
    public static roller(expr: string, rng?: RNG) {
